...
func (dt *DaTong) Finalize(chain consensus.ChainReader, header *types.Header, statedb *state.StateDB, txs []*types.Transaction, uncles []*types.Header) error {
	...
	if numTickets <= 1 {
		...
		return errors.New("Next block doesn't have ticket, wait buy ticket")
	}
	...
	if err != nil {
		return errors.New("UpdateTickets failed: " + err.Error())
	}
	...
	if isInMining {
		...
		if header.MixDigest != hash {
			return fmt.Errorf("MixDigest mismatch, have:%v, want:%v", header.MixDigest, hash)
		}
		if common.IsHeaderSnapCheckingEnabled(header.Number) {
			if !bytes.Equal(getSnapData(header.Extra), snapBytes) {
				return fmt.Errorf("snapBytes in Extra mismatch")
			}
		}
	}
...
func (dt *DaTong) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, statedb *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	if err := dt.Finalize(chain, header, statedb, txs, uncles); err != nil {
		return nil, err
	}
...
func (dt *DaTong) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypeDatong, DatongRLP(header))
	if err != nil {
		return err
	}
...
func (dt *DaTong) getAllTickets(chain consensus.ChainReader, header *types.Header) (common.TicketsDataSlice, error) {
	...
	statedb, err := state.New2(header.Root, header.MixDigest, dt.stateCache)
	if err == nil {
	...
	for {
		...
		statedb, err = state.New2(parent.Root, parent.MixDigest, dt.stateCache)
		if err == nil {
	...
	for i := len(parents) - 1; i >= 0; i-- {
		...
		if number := rawdb.ReadHeaderNumber(dt.db, hash); number != nil {
			receipts := rawdb.ReadReceipts(dt.db, hash, *number, dt.chainConfig)
...
func encodeSigHeader(w io.Writer, header *types.Header) {
	rlp.Encode(w, []interface{}{
		...
		header.GasLimit,
		header.GasUsed,
		new(big.Int).SetUint64(header.Time),
...
func DatongRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (dt *DaTong) calcDelayTime(chain consensus.ChainReader, header *types.Header) (time.Duration, error) {
	...
	delayTime := time.Unix(int64(header.Time+dt.config.Period-2), 0).Sub(time.Now())
	...
	if header.Number.Uint64() > (adjustIntervalBlocks + 1) {
		...
		adjust := ((int64(parent.Time-gparent.Time) / adjustIntervalBlocks) - int64(dt.config.Period)) / adjustIntervalBlocks
		...
		delayTime -= time.Duration(adjust) * time.Second
	}
...
func (dt *DaTong) SetChainConfig(chainConfig *params.ChainConfig) {
	dt.chainConfig = chainConfig
}
